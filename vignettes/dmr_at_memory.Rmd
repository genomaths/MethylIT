...
subtitle: "Analysis for Arabisopsis MSH1-memory versus non-memory first generation"
author: |
    | Robersy Sanchez$^1$
    | rus547@psu.edu
    |
...
concrete physical effect of the DNA, altering the DNA persistance length, i.e.,
...
methylation motifs are not arbibitrally set, but very well constrained
...
samples, equal to 1 (Minlog2FC = 1). Also, it is requested a coeffiecient of 
...
could may want to use lower densities, e.g., CountPerBp = 0.001.
...
All the weight of the evolulationary pressure lie down on naturally ocurring
...
### Differentically methylated genes DMGs
...
It seems to be that metylation patterning inside of gene-body forms a sort
