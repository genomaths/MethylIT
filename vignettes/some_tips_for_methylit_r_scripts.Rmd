...
Plants methylome contains chloroplast and mitochondrial chromosomes, which are not
included in methylome analysis. In the current case, we can exclude these 
chromosomes by setting pattern = "^[1-5]". That is, only rows starting with any 
of the integer numbers from 1 to 5 are read.

Notice that [readCounts2GRangesList](https://is.gd/5pyk1N) function is able to
deal with different file formats. The user just need to provide the number of
the specified columns, which for aligner programs can be different.

It is very important to notice that Methyl-IT pipeline assumes that all the data
provided have aceptable quality. It is up to the user to guaranty the quality of 
its methylome data set. For example, to include chloroplast chromosome, will 
...
1. Theoretically, the nonlinear fit to estimate the probability distribution of 
   ...
   Arbidosis, we have not detected significant differences, so far, when the 
   methylome reprogramming is genome-wide. Nevertheless, if you suspect that the
   methylation reprograming is affecting specific chromosomes, then we strongly
...
2. The cutpoint estimation step is critical. The evaluation of the 
   ...
   with the vignette:  
   [Simple cutpoint estimation for Gamma3P model](https://is.gd/WoYFUe).
