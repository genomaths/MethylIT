...
#'  function is originally provided in the Bioconductor package 'GEOquery'. The
#'  original function download all the supplemental files for a given GEO
#'  accession number. Herein small detail is added to permit only the download
#'  of the specified files and from several GEO accession numbers with only one
...
getGEOSuppFiles <- function(GEO, makeDirectory = FALSE,
    ...
    getGEO <- function(geo, makeDir, baseDir, pattern,
        verbose) {
        ...
        fnames <- try(getDirListing(url = url, verbose = verbose),
            silent = TRUE)
        ...
        if (!is.null(pattern)) {
            idx <- if (length(pattern) > 1) {
                unlist(lapply(pattern, function(p) grep(p, fnames)))
            } else idx = grep(pattern, fnames)
            fnames <- fnames[idx]
        }

        if (makeDirectory) {
            suppressWarnings(dir.create(storedir <- file.path(baseDir,
                geo)))
        }
        for (i in fnames) {
    ...
    x <- lapply(GEO, function(x) {
        getGEO(geo = x, makeDir = makeDirectory, baseDir = baseDir,
