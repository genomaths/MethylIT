...
countTest2 <- function(DS, num.cores = 1, countFilter = TRUE,
    ...
    if (countFilter) {
        ...
        # --------------------- CountPerBp --------------------------- #
        if (!is.null(res) && !is.null(CountPerBp) &&
            is(DS$GR, "GRanges")) {
    ...
    if (!is.null(res)) {
        ...
        ## Var = mean + dispersion * mean ^ 2 weights w = 1
        ## / (Mean + disp * Mean ^ 2)
        if (!is.null(disp)) {
        ...
        # ------ If not individual sample passed the
        # filtering conditions ----- # An empty GRanges
        # object will be returned
        if (is(DS$GR, "GRanges")) {
            res <- GRanges()
            x <- matrix(integer(), nrow = 1, ncol = length(sample.names))
            colnames(x) <- sample.names
            mcols(res) <- DataFrame(x[NULL, ], log2FC = double(),
                pvalue = double(), model = character(),
                adj.pval = double(), CT.SignalDensity = double(),
                TT.SignalDensity = double(), SignalDensityVariation = double())
        } else {
            res <- GRanges()
            x <- matrix(integer(), nrow = 1, ncol = length(sample.names))
            colnames(x) <- sample.names
