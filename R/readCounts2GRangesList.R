    ...
    meth <- lapply(filenames, function(file) {
        ...
        if (grepl(pattern = gz.ext, file)) {
            if (file.exists(gunzippedfile)) {
                text <- paste("A file with the name: \n",
                                gunzippedfile,
                                "\n already exists. So, the corresponding",
                                " file '.gz' wasnot decompressed")
                warning(text, call. = FALSE)
            } else .gunzip(file, remove = FALSE)
        }
        if (.Platform$OS.type == "unix") {
            if (!is.null(pattern)) {
                x <- suppressMessages(fread(paste0("grep ",
                                                pattern, " ", gunzippedfile),
                                            select = columns, ...))
            } else {
                if (!is.null(chromosomes)) {
                    if (length(chromosomes) > 1) {
                        chrom <- paste0("'", paste0(chromosomes,
                                                    collapse = "|"), "'")
                    } else chrom <- chromosomes
                    x <- suppressMessages(fread(paste0("egrep ",
                    chrom, " ", gunzippedfile), select = columns, ...))
        ...
        if (!is.element("end", cn))
            x$end <- x$start
        if (is.element("coverage", cn) && is.element("mC",
            cn)) {
            x$uC <- x$coverage - x$mC
        }
        if (is.element("fraction", cn) && is.element("coverage",
            cn)) {
            x$mC <- round(x$fraction * x$coverage)
            x$uC <- round(x$coverage) - x$mC
        }
        if (is.element("percent", cn) && is.element("coverage",
            cn)) {
