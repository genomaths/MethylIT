...
predict.LogisticR <- function(object, newdata = NULL,
    ...
    # ---This builds data frames from newdata from
    # class 'pDMP' or 'InfDiv' ----#
    if (!is.null(newdata) && (inherits(newdata, "pDMP") ||
        inherits(newdata, "InfDiv"))) {
        ...
        inter <- unlist(lapply(grep("[:]", vn), function(k) strsplit(vn[k],
            split = ":")[[1]]))
        vn <- union(v, inter)
    ...
    if (num.cores > 1 && is.list(dt)) {
        ...
        if (Sys.info()["sysname"] == "Linux") {
            ...
            type = "SOCK")
        newdata <- bplapply(seq_len(length(dt)), function(k) {
