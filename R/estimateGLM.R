...
.estimateGLM <- function(x, groups, baseMV, w, MVrate, 
    ...
    model <- function(dt, model, weights = NULL) {
        if (!is.null(weights)) {
            lev <- levels(dt$group)
            weights <- c(rep(weights[1], sum(groups == 
                lev[1])), rep(weights[2], sum(groups == 
                lev[2])))
        }
        controls <- glm.control(maxit = (10^3), epsilon = (1e-08), 
            trace = FALSE)
        switch(model, Poisson = try(suppressWarnings(glm(count ~ 
            group, family = poisson(link = "log"), 
            data = dt, control = controls)), silent = TRUE), 
            QuasiPoisson = try(suppressWarnings(glm(count ~ 
                group, family = quasipoisson(link = "log"), 
                data = dt, control = controls)), silent = TRUE), 
            Neg.Binomial = try(suppressWarnings(glm.nb(count ~ 
                group, data = dt, control = controls)), 
                silent = TRUE), Neg.Binomial.W = neg_Bin(data = dt, 
