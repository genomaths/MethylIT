...
dmpClusters <- function(GR, maxDist = 3, minNumDMPs = 1,
    ...
    GR$cluster <- paste(seqnames(GR), start(GR), end(GR),
        sep = "_")
    ...
    # To count the number of sites '1' inside each
    # cluster
    if (verbose)
        message("*** Counting DMPs in clusters ...")
    cl <- data.table(as.data.frame(signals))
    cl <- cl[!is.na(cl$cluster), list(seqnames = unique(seqnames),
        start = min(start), end = max(end), dmps = sum(signal)),
        by = cluster]
    cl <- makeGRangesFromDataFrame(data.frame(cl),
        keep.extra.columns = TRUE)
    if (minNumDMPs > 0)
        cl <- cl[cl$dmps >= minNumDMPs]
    return(cl[, "dmps"])
}
...
meth_status <- function(gr, chromosomes = NULL, num.cores = 1L,
    ...
    if (inherits(gr, "GRangesList")) {
        ...
        gr <- uniqueGRanges(gr, chromosomes = chromosomes,
            missing = 0, type = "equal", num.cores = num.cores,
