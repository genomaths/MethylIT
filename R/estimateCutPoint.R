...
estimateCutPoint <- function(LR, control.names, treatment.names,
    ...
    # -------------------------------------------------------------------------
    # # -----------------------Divergences are
    # positives ------------------------# In case that
    # TV column would be used as source to get TVD
    vn <- c("hdiv", "TV")
    ...
    infDiv <- function(LR, div.col = NULL) {
        ...
        dt$ctrl <- data.frame(idiv = abs(mcols(LR$ctrl[,
            div.col])[, 1]), treat = "ctrl")
        dt$treat <- data.frame(idiv = abs(mcols(LR$treat[,
            div.col])[, 1]), treat = "treat")
        return(dt)
    }
    roc <- function(dt) {
    ...
    lcc <- unlist(lapply(control.names, function(k) length(LR[[k]]) >
        0))
    ltt <- unlist(lapply(treatment.names, function(k) length(LR[[k]]) >
        0))
    ...
    if (sum(lcc) == 0) {
        ...
        min.div <- min(unlist(lapply(LR, function(l) min(l@elementMetadata[,
            div.col], na.rm = TRUE))), na.rm = TRUE)
        ...
        classes <- c(rep("CT", length(LR$ctrl)), rep("TT",
            length(LR$treat)))
        classes <- factor(classes, levels = c("CT",
            "TT"))
        ...
        if (simple) {
            ...
            cutp <- roc(dt)
            cutpoint <- cutp$cutp
            predClasses <- mcols(unlist(LR)[, div.col])[,
                1] > cutpoint
            predClasses[predClasses == TRUE] <- "TT"
            predClasses[predClasses == FALSE] <- "CT"
            predClasses <- factor(predClasses, levels = c("CT",
                "TT"))
            ...
            if (inherits(cf.mat, "try-error"))
                stop("*** Cutpoint estimation based on Youden index failed. \n",
                    "Only ", 100 * sum(dt$treat$idiv >
                    cutpoint)/length(dt$treat$idiv),
                    " percent of the cytosine positions from treatment are \n",
                    "greater than the cutpoint value: ",
                    round(cutpoint, 2), "\n\n  Other indicators are: \n",
                    "sensitivity = ", round(cutp$sens,
                    3), "\n", "specificity = ", round(cutp$spec,
                    3), "\n", "auc = ", round(cutp$auc,
            ...
            if (is.null(classifier2[1]))
                classifier2 <- classifier1[1]
            conf.mat <- evaluateDIMPclass(LR, column = column,
                control.names = "ctrl", treatment.names = "treat",
                classifier = classifier1[1], prop = prop,
                output = "conf.mat", n.pc = n.pc, interaction = interaction,
                num.cores = num.cores, tasks = tasks,
                ...)

            post <- predict(object = conf.mat$model,
                newdata = LR, type = "posterior")

            if (classifier1[1] == "logistic" ||
                classifier1[1] == "pca.logistic")
            ...
            if (!is.null(cut.values)) {
                if (is.numeric(cut.values)) {
                    ...
                    if (max(cuts) > max(mcols(unlist(LR)[, div.col])[, 1])) {
                        warning("Cut values supplied are",
                                " > max Inf Div. Ignored")
                        cuts <- cutpoint
                    }
            ...
            if (length(cuts) > 1) {
                ...
                while (k < length(cuts) && !opt && !overcut) {
                    ...
                    if (stat == 12) {
                        ...
                        if (k == 1) {
                            st <- conf.mat$Performance$byClass[stat]
                            warning("Model classifier ",
                                    classifier1[1], " is enough")
                        }
                    }
                }
            } else {
...
print.CutPoint <- function(x, digits = getOption("digits"),
    ...
    postProbCut <- format(signif(x$postProbCut, max(1L,
        digits - 2L)))
    cutpoint <- format(signif(x$cutpoint, max(1L, digits -
        2L)))
    ...
    if (!is.na(x$statistic)) {
        ...
        sta.val <- format(signif(x$optStatVal, max(1L,
            digits - 2L)))
